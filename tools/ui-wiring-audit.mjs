...
for (const f of htmlFiles) {
  ...
  for (const m of txt.matchAll(/\bid\s*=\s*["']([^"']+)["']/g)) { const id=m[1].trim(); if(/^[A-Za-z][\w:-]*$/.test(id)) addRef(htmlIds,id,f); }
  for (const m of txt.matchAll(/\bclass\s*=\s*["']([^"']+)["']/g)) {
    m[1].split(/\s+/).filter(Boolean).forEach(c=>{ if(/^[A-Za-z][\w:-]*$/.test(c)) addRef(htmlClasses,c,f); });
  }
}
...
for (const f of cssFiles) {
  const txt = fs.readFileSync(f,'utf8');
  for (const m of txt.matchAll(/(^|[^0-9A-Fa-f])#([A-Za-z_][\w-]*)/g)) { const id=m[2]; if(!/^[0-9a-fA-F]{3,6}$/.test(id)) addRef(cssIdSelectors,id,f); }
...
for (const f of jsFiles) {
  ...
  for (const m of txt.matchAll(/querySelector(?:All)?\(\s*["']#([A-Za-z_][\w-]*)["']\s*\)/g)) addRef(jsIdRefs,m[1],f);
  for (const m of txt.matchAll(/querySelector(?:All)?\(\s*["']\.([A-Za-z_][\w-]*)["']\s*\)/g)) addRef(jsClassRefs,m[1],f);
  for (const m of txt.matchAll(/classList\.(?:add|remove|toggle|contains)\(\s*["']([^"']+)["']/g)) addRef(jsClassRefs,m[1],f);
}
...
lines.push('');
lines.push('## Summary');
const status = idsOnlyHtml.length===0 && idsJsMissingHtml.length===0 && idsCssMissingHtml.length===0 && classesOnlyHtml.length===0 && classesJsMissingHtml.length===0 && classesCssMissingHtml.length===0 ? 'PASS' : 'ISSUES FOUND';
lines.push(`Overall status: **${status}**`);
lines.push('');
function section(title, list, map){
  lines.push(`## ${title} (${list.length})`);
  if(list.length===0){lines.push('- None'); lines.push(''); return;}
