...
function extractRefsFromCode(code, idsMap, classesMap, file) {
  ...
  // If the call argument looks like a selector (e.g. '#id', '.class'), treat it as selector syntax.
  for (const m of code.matchAll(/\$\(\s*["']([^"']+)["']\s*\)/g)) {
    const arg = m[1].trim();
    if (/^[#.\[]/.test(arg) || /\s|[>~+:]/.test(arg)) extractSimpleSelectorRefs(arg, idsMap, classesMap, file);
    else addRef(idsMap, arg, file);
  }
