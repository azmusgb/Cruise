    (function () {
      ...
      // =========================================================
      const SAIL_START = new Date('2026-02-14T16:00:00'); // local device time
      ...
      function getExcursions(){
        return safeJSON(localStorage.getItem(STORAGE_EXCURSIONS), {});
      }
      ...
      function openDay(idx, scrollIntoView=true, opts={announce:true, fromSync:false, allowSailaway:true}) {
        ...
        // Announce
        if (opts.announce) announce(`Expanded details for Day ${idx+1}: ${ITIN[idx].label}.`);
      ...
      function toggleCard(card){
        ...
        if (expanded) {
          ...
          announce(`Collapsed details for Day ${idx+1}: ${ITIN[idx].label}.`);
        } else {
          openDay(idx, true);
        }
      }
